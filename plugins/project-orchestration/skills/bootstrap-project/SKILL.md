...
For new bounded work, prefer opt-in `## Handoff Contract v1` sections. Define stable `G-###`, `NG-###`, `INV-###`, `D-###`, and `AC-###` IDs in the immutable Requirement Brief, reference the needed IDs once in the Work Item, and map every referenced ID in the committed Evidence `## Traceability` table to a non-empty output, verification, and evidence pointer. `start` rejects malformed, duplicate, or Brief-unknown IDs; `handoff` rejects missing, duplicate, unknown, or incomplete rows. Briefs and Work Items without v1 remain compatible. Use pseudocode or decision tables when behavior needs precision, but keep them explanatory rather than inventing another authority format.
